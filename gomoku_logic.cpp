...
extern "C" int secondary_diagonal_check(int last_move[2], int field_info[YSIZE*XSIZE])
{
	const int x = last_move[0] - 4;
	const int y = last_move[1] + 4;
	...
	for (int i = 0; i < 9; ++i)
	{
		if (x+i < 0 || y-i >= YSIZE) continue;
		if (x+i >= XSIZE || y-i < 0) break;
...
extern "C" int find_winner(int last_move[2], int field_info[YSIZE*XSIZE])
{
	...
	winner = diagonal_check(last_move, field_info); if (winner!=0) return winner;
	return secondary_diagonal_check(last_move, field_info);
}
